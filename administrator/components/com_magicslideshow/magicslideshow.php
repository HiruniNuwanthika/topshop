...
if(method_exists('JUser', 'authorise')) {
    if(!JFactory::getUser()->authorise('core.manage', 'com_magicslideshow')) {
        return JError::raiseWarning(404, JText::_('JERROR_ALERTNOAUTHOR'));
    }
} else {
    //NOTE: For Joomla 1.5.x
    $acl = JFactory::getACL();
    $acl->addACL('com_magicslideshow', 'manage', 'users', 'super administrator');
    $acl->addACL('com_magicslideshow', 'manage', 'users', 'administrator');
    if(!JFactory::getUser()->authorize('com_magicslideshow', 'manage')) {
...
if(method_exists('MagicToolboxLegacyController', 'getInstance')) {
    ...
    $controller = MagicToolboxLegacyController::getInstance('Magicslideshow');
} else {
    //NOTE: For Joomla 1.5.x
    ...
    //$controller = new MagicslideshowController(array('default_task' => 'display'));
    $classname  = 'MagicslideshowController'.ucfirst($controller);
